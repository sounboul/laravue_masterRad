<template>
  <div class="app-container">
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="120px" align="center" label="Brand">
        <template slot-scope="scope">
          <span>{{ scope.row.brand }}</span>
        </template>
      </el-table-column>

      <el-table-column width="100px" label="Rating">
        <template slot-scope="scope">
          <svg-icon v-for="n in +scope.row.rating" :key="n" icon-class="star" class="meta-item__icon" />
        </template>
      </el-table-column>

      <el-table-column class-name="status-col" label="Amount" width="110">
        <template slot-scope="{row}">
          <el-tag :type="row.amount | statusFilter">
            {{ row.amount }}
          </el-tag>
        </template>
      </el-table-column>

      <el-table-column min-width="300px" label="Article">
        <template slot-scope="{row}">
